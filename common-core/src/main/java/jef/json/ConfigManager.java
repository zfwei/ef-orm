...
public final class ConfigManager {
	static Map<String,SerializeConfigEx> typeKeys=new HashMap<String,SerializeConfigEx>();
	
	public static SerializeConfig getGlobal(){
		return SerializeConfig.getGlobalInstance();
	}
	
	public static SerializeConfigEx get(String key){
		SerializeConfigEx config=typeKeys.get(key);
		if(config==null){
			synchronized (typeKeys) {
				//A copy on write Map.
				Map<String,SerializeConfigEx> c=new HashMap<String, SerializeConfigEx>(typeKeys);
				config=new SerializeConfigEx();
